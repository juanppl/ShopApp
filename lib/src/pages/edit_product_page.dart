...
class _EditProductPageState extends State<EditProductPage> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      if (_productId != null) {
        _createdProduct =
            Provider.of<ProductsProvider>(context).findProductById(_productId);
        _initData = {
  ...
  void _saveFormData() async {
    ...
    if (_productId == null) {
      ...
      try {
        await Provider.of<ProductsProvider>(context, listen: false).addProduct(_createdProduct);
      } catch (error) {
      ...
      try{
        await Provider.of<ProductsProvider>(context, listen: false).updateProduct(_productId, _createdProduct);
      }catch (error) {
        await showDialog<Null>(
            context: context,
            builder: (_) {
                  ...
                  children: <Widget>[
                    ...
                    TextFormField(
                      ...
                      onSaved: (value) {
                        ...
                        _data.addAll({'price': double.parse(value)});
                      },
                      validator: (value) {
                        if (value.length < 0) {
